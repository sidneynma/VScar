...
const statusLabel: Record<VehicleParty["status"], string> = {
  active: "Ativo",
  inactive: "Inativo",
}
            ...
            <div className="space-y-2">
              {parties.map((party) => (
                ...
                >
                  ...
                  <p className="text-xs" style={{ color: "var(--text-muted)" }}>Status: {statusLabel[party.status]}</p>
                </button>
              ))}
            </div>
          )}
              ...
              <div className="rounded-lg p-3 mb-4" style={{ backgroundColor: "var(--bg-secondary)" }}>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <div className="flex items-center gap-2">
                    <span className={selectedParty.status === "active" ? "badge badge-green" : "badge badge-gray"}>{statusLabel[selectedParty.status]}</span>
                    <Link href={`/dashboard/vehicle-parties/${selectedParty.id}`} className="btn-secondary" style={{ padding: "0.2rem 0.5rem", fontSize: "0.75rem" }}>Editar</Link>
                  </div>
                </div>
